...
export default function App() {
  ...
  async function fetchDocs() {
    // /documents endpoint not yet in backend — multi-doc support coming in Phase 2
    setDocuments({});
  }
  ...
  async function handleUpload(file) {
    ...
    try {
      ...
      const d = await r.json();
      setUploadMsg({ type: "success", text: `Indexed successfully! Re-index any time by uploading again.` });
      // Show a placeholder entry in the docs tab so the user sees feedback
      setDocuments({ "current": { filename: file.name, num_pages: "—" } });
    } catch {
      ...
      <div style={{
        ...
        {(tab === "search" || tab === "ask") && (
          <div style={{ animation: "fadeSlideIn 0.3s ease" }}>
            {/* Mode toggle hidden — /ask RAG endpoint coming in Phase 3 */}
            ...
            {/* RAG answer bubble hidden — /ask endpoint coming in Phase 3 */}
            ...
            {!loading && results.length > 0 && (
              <div>
                ...
                <div style={{
                  ...
                  {results.map((r, i) => <PageCard key={r.page_key ?? r.page} result={r} index={i} />)}
                </div>
              </div>
            )}
